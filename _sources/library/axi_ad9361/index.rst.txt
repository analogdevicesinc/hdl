...
* :git-hdl:`Interface <library/axi_ad9361/axi_ad9361.v#L302>` module in either
  CMOS Dual Port Full Duplex or LVDS mode for
  :git-hdl:`Intel <library/axi_ad9361/intel>` or
  :git-hdl:`AMD Xilinx <library/axi_ad9361/xilinx>` devices.
* :git-hdl:`Receive <library/axi_ad9361/axi_ad9361_rx.v>` module, which
  contains:

  * :git-hdl:`ADC channel processing <library/axi_ad9361/axi_ad9361_rx_channel.v>` modules, one for each
    channel
    ...
    * :git-hdl:`ADC PN Monitor <library/axi_ad9361/axi_ad9361_rx_pnmon.v>`
...
* :git-hdl:`Transmit <library/axi_ad9361/axi_ad9361_tx.v>` module, which
  contains:

   * :git-hdl:`DAC channel processing <library/axi_ad9361/axi_ad9361_tx_channel.v>` modules, one for each
     channel

     * Different data generators (:git-hdl:`DDS <library/common/ad_dds.v>`,
       pattern, PRBS)
...
#. Interface Logic Only
#. Disable DSP Functions
#. Removing AXI interface and Processor Control
...
like ADC common, ADC channel, DAC common, DAC channel etc.
The following table presents the base addresses of each instance, after that can
be found the detailed description of each generic register map.
...
see :ref:`ADC register access <generic-adc-register-access>` and
:ref:`DAC register access <generic-dac-register-access>`.
...
.. list-table:: Register Map base addresses for axi_ad9361
   ...
   * - 0x2000
     - 0x8000
     - TDD CONTROL
     - See the `Transceiver TDD Control <#hdl-regmap-TDD_CNTRL>`__ table for
       more details.
...
The IP was developed as part of the :ref:`fmcomms2` reference design.

The control of the AD9361 is done through an SPI interface, which is needed at
the system level. The ENABLE and TXNRX signals must be connected at the system
level to control the ENSM state machine. In TDD mode, these are driven by the
integrated TDD controller; see :ref:`axi_ad9361 tdd` for details.

The *ADC/DAC interface signals* must be connected directly to the top file of
the design, as IO primitives are part of the IP.

The example designs use a DMA to move data between the IP and memory. If the
data needs to be processed in HDL before moving to memory, this can be done at
the output of the IP (at system level) or inside the ADC/DAC channel module
(at IP level).

The example designs use a processor to program all the registers. If no
...
The software for this IP can be found as part of the :ref:`fmcomms2` reference
design.

The AD9361 Linux driver uses the IIO framework and exposes the transceiver as
an IIO device. When TDD mode is used, the TDD controller is instantiated as a
separate IIO device. Devicetree configuration is required to instantiate the
TDD core driver; refer to :ref:`axi_ad9361 tdd` for the required properties
and example.

To switch between FDD and TDD modes, the ``adi,frequency-division-duplex-mode-enable``
devicetree property must be set accordingly. In TDD mode, the
``TDD_DISABLE`` HDL parameter must be set to 0 (default) to enable the
integrated TDD controller.
