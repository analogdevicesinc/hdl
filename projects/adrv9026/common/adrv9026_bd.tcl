...
if {$ADI_PHY_SEL == 1} {
  ...
  set REF_CLK_RATE [ expr { [info exists ad_project_params(REF_CLK_RATE)] \
                            ? $ad_project_params(REF_CLK_RATE) : 250 } ]

  create_bd_port -dir I gt_reset

  create_versal_phy jesd204_phy $TX_NUM_OF_LANES {10} {10} $REF_CLK_RATE {RXTX}
...
if {$ADI_PHY_SEL == 1} {
  ...
  ad_connect $rx_ref_clk jesd204_phy/GT_REFCLK
  ad_connect gt_reset jesd204_phy/gtreset_in
    ...
    for {set j 0}  {$j < $RX_NUM_OF_LANES} {incr j} {
      ad_connect  axi_adrv9026_rx_jesd/rx_phy${j} jesd204_phy/rx${j}
    }
    ...
    for {set j 0}  {$j < $TX_NUM_OF_LANES} {incr j} {
      ad_connect  axi_adrv9026_tx_jesd/tx_phy${j} jesd204_phy/tx${j}
    }
...
if {$ADI_PHY_SEL == 1} {
  ...
  make_bd_intf_pins_external  [get_bd_intf_pins jesd204_phy/GT_Serial_0]
}