...
# rename auto-generated clock for SPIEngine to spi_clk
# spi_clk is derived from qadc_mclk_refclk (16.384MHz) through the MMCM: 16.384 * 42 / 6 ~ 114.688MHz
...
# execution's shift register. New parallel data loads only on (transfer_active && trigger_tx && first_bit)
# The trigger_tx fires once every (clk_div+1)*2 spi_clk cycles, and first_bit is true only once per word. So
# the register is loaded once every word_length * 2 * (clk_div+1) cycles. With DATA_WIDTH=32 and even clk_div=0,
# that's every 64 spi_clk cycles. An 8-cycle multicycle is a conservative constraint and still helpful for timing closure
...
## Just copied the multicycle path values from the data_sdo_shift_reg to the left_aligned_reg
# any value >= 2 would be fine since this register is written in a config write instruction and used many
# cycles later during the transfers.
