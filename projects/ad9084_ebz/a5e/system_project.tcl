...
adi_project ad9084_ebz_a5e [list \
  JESD_MODE           [get_env_param JESD_MODE        8B10B ] \
  REF_CLK_RATE        [get_env_param REF_CLK_RATE    168.75 ] \
  DEVICE_CLK_RATE     [get_env_param DEVICE_CLK_RATE 168.75 ] \
  RX_LANE_RATE        [get_env_param RX_LANE_RATE      6.75 ] \
  TX_LANE_RATE        [get_env_param TX_LANE_RATE      6.75 ] \
...
foreach half {a b} {
  for {set j 0} {$j < $rx_num_lanes} {incr j} {
    foreach pin [list rx_data_${half}_p[$j] rx_data_${half}_n[$j]] {
      ...
      set_instance_assignment -name HSSI_PARAMETER "rx_adaptation_mode=RX_ADAPTATION_MODE_FLUX_ADAPTATION" -to $pin
    }
  }
}
