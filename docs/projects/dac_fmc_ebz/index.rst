...
For lower sample rates, the DAC FIFO can be placed in bypass mode, in which
case the DMA must stream the data from the PS memory.
...
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
For each channel, software can set the values that will be driven to the DAC.
...
|                 |(from FPGA view) +--------+-------+ Zynq MP       | Microblaze    | Arria® 10     |
|                 |                 | Xilinx | Intel |               |               |               |
+=================+=================+========+=======+===============+===============+===============+
...
Then go to the **/projects/dac_fmc_ebz/zcu102** location and run the make
command by typing in your command prompt:

Default configuration (ADI_DAC_DEVICE=AD9172 ADI_DAC_MODE=04):
...
If the AD9164 device is needed and the mode should be 09, the following
