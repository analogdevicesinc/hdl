...
The :adi:`M2K <ADALM2000>` is built around the XC7Z010 Xilinx Zynq FPGA.
...
interface to the :adi:`AD9963` ADC and a 14 pin interface to the :adi:`AD9963`
...
.. collapsible:: PS7
   ...
   In order to minimize the power utilization for the design, the PS7 block is
   ...
   27MHz clock (FCLK0) for the AXI interface, the logic generator and pattern
   generator DMAs, a 100MHz clock (FCLK2) for the logic analyzer and pattern
   generator path and a 55MHz clock for the oscilloscope DMA and one of the two
   analog waveform generator DMAs.
...
.. collapsible:: AXI_AD9963
   ...
   in the FPGA. In order to reduce the number of PLL used in the FPGA, we are
   ...
   More information: :ref:`AXI AD9963 documentation <axi_ad9963>`
...
.. collapsible:: AXI_ADC_TRIGGER
   ...
   The output of the core embeds the triggers in the data words, as only 12
   ...
   More information: :ref:`AXI ADC TRIGGER documentation <axi_adc_trigger>`

.. collapsible:: UTIL_VAR_FIFO

   The :ref:`UTIL_VAR_FIFO <util_var_fifo>` IP allows M2K to store and display
   data before the trigger.
   In the design, it has a length of 8192 and can easily be increased. The IP
   controls an external BRAM. If ``DEPTH`` is 0, the FIFO is bypassed. When the
   ``DEPTH`` is changed, the read and write pointers are reset, effectively
   resetting the FIFO. There is a 2 cycle clock latency, even if it's bypassed.
   If valid is not always asserted (decimation is active), the latency is one
   clock cycle instead of two.
   
   More information: :ref:`UTIL VARIABLE FIFO documentation <util_var_fifo>`

.. collapsible:: UTIL_EXTRACT

   The :ref:`UTIL_EXTRACT <util_extract>` IP will extract the trigger that was
   embedded in the data stream by the :ref:`AXI_ADC_TRIGGER <axi_adc_trigger>`
   IP.

   More information: :ref:`UTIL EXTRACT documentation <util_extract>`
...
.. collapsible:: AXI_ADC_DECIMATE
   ...
   More information: :ref:`AXI ADC DECIMATE documentation <axi_adc_decimate>`
...
.. collapsible:: AXI_DAC_INTERPOLATE
   ...
   More information: :ref:`AXI DAC INTERPOLATION documentation <axi_dac_interpolate>`

.. collapsible:: AXI_LOGIC_ANALYZER

   The :ref:`AXI_LOGIC_ANALYZER <axi_logic_analyzer>` IP implements both the
   ...
   In order to provide data before triggering, a variable length FIFO is used
   ...
   More information: :ref:`AXI LOGIC ANALYZER documentation <axi_logic_analyzer>`
...
- :git-linux:`ZYNQ M2K (Z7010) Linux device tree <arch/arm/boot/dts/xilinx/zynq-m2k.dts>`
- :git-linux:`ZYNQ-ZED-ADV7511-M2K Rev.B Linux device tree <arch/arm/boot/dts/xilinx/zynq-zed-adv7511-m2k-revb.dts>`
- :git-linux:`ZYNQ-ZED-ADV7511-M2K Rev.C Linux device tree <arch/arm/boot/dts/xilinx/zynq-zed-adv7511-m2k-revc.dts>`
