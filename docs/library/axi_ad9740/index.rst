...
:adi:`AD9748` devices.
It is a DAC with 8, 10, 12 or 14 bits resolution and with sample rates up to
210 MSPS (per datasheet). The reference design uses a 250 MHz external clock
from the :adi:`ADF4351`, achieving up to 250 MSPS DAC update rate using DDR
output. This documentation only covers the IP core and requires that one must
be familiar with the device for a complete and better understanding.
...
* 32-bit DMA data interface (2 x 16-bit samples per transfer)
* DDR output via Xilinx ODDR primitives (CLK_RATIO=2)
...
* Data format conversion (two's complement to offset binary via MSB inversion)
* MSB-aligned 14-bit data path for all resolutions, with unused lower bits
  held at zero
...
.. list-table::
   ...
   * - :git-hdl:`library/axi_ad9740/axi_ad9740_if.v`
     - Verilog source for the AXI AD9740 DDR output interface.
...
* :git-hdl:`Interface <library/axi_ad9740/axi_ad9740_if.v>`
  module for Xilinx devices. Uses ODDR primitives to output two 14-bit samples
  per clock cycle in DDR mode (CLK_RATIO=2). Handles data format conversion
  (two's complement to offset binary) by inverting the MSB when enabled.
  For DAC resolutions below 14 bits, unused lower output bits are driven to
  zero through dedicated ODDR instances.
...
.. hdl-parameters::
   ...
   * - CLK_RATIO
     - Number of samples processed per clock cycle. Fixed at 2 for DDR output
       (two samples per clock via ODDR primitives). This parameter determines
       the DMA data bus width (32 bits) and the internal data path width
       (28 bits). Default value is 2.
   * - DDS_CORDIC_DW
     - CORDIC DDS data width, range 8-24. Default value is 14. The reference
       design sets this to 22 for higher precision.
   * - DDS_CORDIC_PHASE_DW
     - Number of CORDIC rotation stages, range 8-24. Higher values provide
       better precision at the cost of more resources. Default value is 14.
       The reference design sets this to 22.
...
.. hdl-interfaces::
   ...
   * - dac_clk
     - Input clock for the DAC interface. Drives the ODDR output primitives
       and all DAC-domain logic.
   * - s_axis
     - AXI4-Stream slave interface for DMA data input. 16*CLK_RATIO bits wide
       (32 bits for CLK_RATIO=2), carrying two 16-bit samples per transfer.
       Connected to the DMA FIFO in the reference design.
   * - dac_data
     - 14-bit DDR output to DAC via ODDR primitives. For DAC resolutions
       below 14 bits, the lower (14 - DAC_RESOLUTION) bits are held at zero.
       Active data bits are MSB-aligned.
   * - s_axi
...
The *DAC data output signals* must be directly connected to the top-level FPGA
I/O pins. The ``dac_clk`` input expects a logic-rate clock (e.g. the external
clock divided by 2 via BUFR), not the raw pad clock. The reference design uses
IBUFGDS followed by a BUFR divide-by-2 to derive the logic clock from the
250 MHz differential input.
...
* Sedation mode (0x3) - Zero output (mute)
...
binary format expected by these devices by inverting the MSB of each sample.
The lower bits are passed through unchanged.
