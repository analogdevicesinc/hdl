...
The :git-hdl:`AXI HMCAD15XX <library/axi_hmcad15xx>` IP core can be used to interface
the :adi:`HMCAD1511, HMCAD1520` devices.
...
.. hdl-parameters::
   ...
   * - FPGA_FAMILY
     - Used to select the family variant of the FPGA device, auto set in project.
   * - SPEED_GRADE
     - Used to set the FPGA’s speed-grade
   * - DEV_PACKAGE
     - Value describing the device package. The package might affect high-speed interfaces
...
.. hdl-interfaces::
   ...
   * - adc_enable_0
     - Enable signal for ADC 0
   * - adc_enable_1
     - Enable signal for ADC 1
   * - adc_enable_2
     - Enable signal for ADC 2
   * - adc_enable_3
     - Enable signal for ADC 3
...
* The LVDS data is deserialized by the
  ...
  a 1:8 ratio.
* The data lanes are processed by :git-hdl:`sample_assembly<library/hmcad15xx/sample_assembly.v>` based
  on resolution and frame_data. For 8-bit resolution data gets packed into 8-bit words, for 12/14 bit resolution
  data gets packed into 16-bit words.
* 14-bit resoltion works only in Quad Channel Mode.
* The bits in the sample are inverted by polarity_mask_s
* All of the samples are sent at the same time on adc_data when adc_valid asserts.
...
The control of the HMCAD15xx chip is done through a SPI interface, which is needed
at system level.
...
.. list-table:: Main registers used to control the AXI HMCAD15xx IP
   ...
   * - RESOLUTION
     - 0x4C (ADC Common)
     - [1:0]
     - This value is used to select the resolution for the ADC. (2'b00 -> 8-bit resolution, 
   ...
   * - POLARITY_MASK_S
     - 0x80 (ADC Common)
     - [7:0]
     - Used to invert data coming from each lane. (E.g. polarity_mask_s == 8'b11010000 => bits coming on lanes 7, 6 and 4 will be inverted)
