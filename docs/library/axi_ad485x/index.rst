...
The :git-hdl:`AXI AD485x <library/axi_ad485x>` IP core
can be used to interface the :adi:`AD485x` devices.
...
.. hdl-parameters::
   ...
   * - DEVICE
     - The device to interface with AD4858x where x can be 1 to 8
   * - DW
     - Data Width parameter is auto selected based on DEVICE.
   * - LANE_0_ENABLE
     - Lane 0 enable, for CMOS if. signals that a lane is connected or not.
   * - LANE_1_ENABLE
     - Lane 1 enable, for CMOS if. signals that a lane is connected or not.
   * - LANE_2_ENABLE
     - Lane 2 enable, for CMOS if. signals that a lane is connected or not.
   * - LANE_3_ENABLE
     - Lane 3 enable, for CMOS if. signals that a lane is connected or not.
   * - LANE_4_ENABLE
     - Lane 4 enable, for CMOS if. signals that a lane is connected or not.
   * - LANE_5_ENABLE
     - Lane 5 enable, for CMOS if. signals that a lane is connected or not.
   * - LANE_6_ENABLE
     - Lane 6 enable, for CMOS if. signals that a lane is connected or not.
   * - LANE_7_ENABLE
     - Lane 7 enable, for CMOS if. signals that a lane is connected or not.
   * - ECHO_CLK_EN
     - Enables echoed clock for capturing data in other words enables the SSI
   * - EXTERNAL_CLK
...
.. hdl-interfaces::
   ...
   * - busy
     - Device busy signal
   * - lvds_cmos_n
     - Slects the interface to be used by the device
   * - scki_p
   ...
   * - external_clk
     - External clock for internal logic and data path(optional for CMOS),
       enabled by EXTERNAL_CLK parameter. If not enable s_axi_aclk will be used
   * - adc_enable_*
...
of interest. It is then used by the PACK cores to route the data based on total
number of channels and the selected number of channels. As an example,
AXI_AD485x supports a total of 8(or 4) channels 32(or 16) bits each.
...
.. note::
   ...
   The axi_ad4858 is used in conjunction with axi_pwm_gen which is responsible for
...
.. warning::
   ...
   to enable internal clock buffers in AD4858
...
like ADC common, ADC channel, up_delay_ctrl. The following table presents the
base addresses of each instance, after that can be found the detailed
description of each generic register map. The absolute address of a register
should be calculated by adding the instance base address to the registers
relative address.
...
Reference Design at: :git-no-OS:`projects/ad485x` (TBD)
...
* :adi:`AD4851`
* :git-linux:`/`
* :git-no-OS:`projects/ad485x`
* :dokuwiki:`AD4858 Reference Design <resources/eval/user-guides/ad4858_fmcz/ad4858_fmcz_hdl>`
