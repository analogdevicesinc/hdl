...
//   reg        [31:0] gpio_dir_reg = 32'hffffffe7;  // 0 = output, 1 = input
//   reg        [31:0] up_rdata;
//   reg        up_rack = 'h0;
//   reg        up_wack = 'h0;
...
//       gpio_out_reg <= 32'b0;
//       gpio_dir_reg <= 32'hffffffe7;  // Fixed direction: bits 0-2 input, bits 3-4 output
//     end else if (up_wreq_s == 1'b1) begin
//       case (up_waddr_s)
//         8'h21: gpio_out_reg <= up_wdata_s;
//         // Direction register is now read-only (fixed)
//       endcase
//     end
//   end
...
//     up_resetn <= 1'b0;
// //    gpio_out  <= 32'b0;
// //    gpio_tri  <= 32'hfffffff5;  // 2 butoane input, 2 LED-uri output
//     up_wack   <= 1'b0;
//   end else begin
//     up_wack <= up_wreq_s;
//     if (up_resetn == 1'b0) begin
//       // Reset intern activ
//       gpio_out <= 32'b0;
//       gpio_tri <= 32'hfffffff3;
//     end else if (up_wreq_s == 1'b1) begin
//       case (up_waddr_s)
//         8'h20: up_resetn <= up_wdata_s[0]; // reset controlat din software
//         8'h21: gpio_out  <= up_wdata_s;    // scriere LED-uri
//       endcase
//     end
//   end
// end
...
//          8'h21: up_rdata[31:0] <= gpio_out_reg[31:0];
          //rename
          8'h21: up_rdata[31:0] <= gpio_i[31:0]; // data input
          //8'h22: up_rdata[31:0] <= gpio_out[31:0];




          //rename to tristate
          8'h24: up_rdata[31:0] <= gpio_tri[31:0];// se direction
  ...
  //limitation irq for output, when irq is output not consider
  assign gpio_edge_detect = (gpio_in  & (~gpio_in_d1)) & gpio_t;
