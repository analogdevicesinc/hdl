...
/* (* mark_debug = "true" *) */  reg        gate_en_req;       // combinational
  ...
  // with the XTAL2_CLKIN falling edge (docx image16, digital-designer
  // recommendation). Bench A/B (ad7134_phase_8ch.py, ~20 boots): negedge is
  // deterministic but out of spec (~+29.9 ns); posedge lands the inter-chip offset
  // at ~+7.9 ns (~98-100% inside the <10 ns spec). Datasheet root cause: the chip
  // samples the ODR RISING edge against its internal XTAL2/2 (24 MHz HP) grid, so
  // a half-CLKIN-cycle shift moves the ODR edge into the "good bin" away from the
  // metastable sampling point.
  //
  // TO REVERT to the RISING-edge (posedge) variant: uncomment the posedge flops
  // below, comment the negedge flops, AND swap back to the -max 5.500 / -min 15.900
  // pair on the ad713x_odr set_output_delay in system_constr.xdc.
  ...
  // Rising-edge debug twin (kept for easy revert):
  // always @(posedge clk_in) begin
  //   odr_out_dbg <= odr_in;
  // end
  always @(negedge clk_in) begin
