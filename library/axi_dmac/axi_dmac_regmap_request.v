...
module axi_dmac_regmap_request #(
  ...
  assign up_dma_cyclic_clear = (up_wreq == 1'b1 && up_waddr == 9'h103) ? ~up_wdata[0] & DMA_CYCLIC : 1'b0;
  ...
  always @(posedge clk) begin
    if (reset == 1'b1) begin
      ...
      if (ctrl_enable == 1'b1) begin
        ...
        end else if (up_dma_cyclic_clear && up_dma_cyclic) begin
          up_dma_req_valid <= 1'b0;
        end else if (up_sot && !up_dma_cyclic) begin
          up_dma_req_valid <= 1'b0;
        end
      end else begin
        up_dma_req_valid <= 1'b0;
  ...
  if (DMA_CYCLIC == 1) begin
    ...
    reg [1:0] cyclic_queued = 2'h0;
    wire [1:0] cyclic_queued_next;

    assign cyclic_queued_next = (request_valid_sot && !response_valid_eot) ? cyclic_queued + 1'b1 :
                                (response_valid_eot && !request_valid_sot && cyclic_queued != 2'h0) ?
                                cyclic_queued - 1'b1 : cyclic_queued;
    assign cyclic_transfer_s = cyclic_queued != 2'h0;

    always @(posedge clk) begin
      if (ctrl_enable == 1'b0) begin
        cyclic_queued <= 2'h0;
      end else if (up_dma_cyclic == 1'b1) begin
        cyclic_queued <= cyclic_queued_next;
      end else if (cyclic_queued > 2'h1) begin
        cyclic_queued <= cyclic_queued_next;
      end else begin
        cyclic_queued <= 2'h0;
      end
