...
module axi_dmac_regmap_request #(
  ...
  if (DMA_CYCLIC == 1) begin
    ...
    wire cyclic_transfer;

    assign cyclic_transfer = up_dma_cyclic | |cyclic_queued;
    assign cyclic_queued_next = (request_valid_sot && !response_valid_eot) ? cyclic_queued + 1'b1 :
                                ...
                                cyclic_queued;
    assign cyclic_transfer_s = (cyclic_queued != 2'h0) && (cyclic_queued_next != 2'h0);

    always @(posedge clk) begin
      if (ctrl_enable == 1'b0) begin
        cyclic_queued <= 2'h0;
      end else if (cyclic_transfer) begin
