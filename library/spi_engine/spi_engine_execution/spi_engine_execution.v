...
module spi_engine_execution #(
  ...
  localparam DEFAULT_MASK_SPI_LANE = (2 ** NUM_OF_SDI) - 1; //by default all lanes are enabled
  ...
  reg [7:0] spi_lane_mask =  DEFAULT_MASK_SPI_LANE;
  ...
  spi_engine_execution_shiftreg #(
    ...
    .clk(clk),
    .resetn(resetn),
    .s_interconnect_dir(s_interconnect_dir),
    .sdi(sdi),
    .sdo_int(sdo_int_s),
    .echo_sclk(echo_sclk),
    .sdo_data(sdo_data),
  ...
  always @(posedge clk) begin
    if (resetn == 1'b0) begin
      ...
      left_aligned <= 8'b0;
      spi_lane_mask <= DEFAULT_MASK_SPI_LANE;
