...
module rtp_engine_regmap (
  ...
  output      [15:0]                         dest_addr_hb,
  output      [31:0]                         dest_addr_lb,
  output      [15:0]                         source_addr_hb,
  output      [31:0]                         source_addr_lb,
  ...
  reg  [15:0]     up_dest_addr_hb = 'h0;
  reg  [31:0]     up_dest_addr_lb = 'h0;
  reg  [15:0]     up_source_addr_hb = 'h0;
  reg  [31:0]     up_source_addr_lb = 'h0;
  ...
  reg  [5:0]      up_fps = 'h0;
  reg             up_convert_uyvy_to_yuyv = 1'b0;
  ...
  always @(posedge up_clk) begin
    if (!up_rstn) begin
      ...
      up_num_px_p_line <= 'h0;
      up_fps <= 'h0;
      up_convert_uyvy_to_yuyv <= 1'b0;
    end else begin
      up_wack <= up_wreq;
      if ((up_wreq == 1'b1) && (up_waddr == 14'h1)) begin
        up_dest_addr_lb<= up_wdata[31:0];
      end
      if ((up_wreq == 1'b1) && (up_waddr == 14'h2)) begin
        up_dest_addr_hb <= up_wdata[15:0];
      end
      if ((up_wreq == 1'b1) && (up_waddr == 14'h3)) begin
        up_source_addr_lb <= up_wdata[31:0];
      end
      if ((up_wreq == 1'b1) && (up_waddr == 14'h4)) begin
        up_source_addr_hb <= up_wdata[15:0];
      end
      ...
      if ((up_wreq == 1'b1) && (up_waddr == 14'hc)) begin
        up_convert_uyvy_to_yuyv <= up_wdata[0];
      end
    end
  end
      ...
      if (up_rreq) begin
        if (up_raddr[13:5] == 10'd0) begin
          case (up_raddr) inside
            ...
            14'h1: up_rdata <= up_dest_addr_lb;
            14'h2: up_rdata <= {15'd0,up_dest_addr_hb};
            14'h3: up_rdata <= up_source_addr_lb;
            14'h4: up_rdata <= {15'd0,up_source_addr_hb};
	    ...
	    14'hb: up_rdata <= {26'd0, up_fps};
            14'hc: up_rdata <= {31'd0, up_convert_uyvy_to_yuyv};
	    default: up_rdata <= 0;
          endcase
        end else begin
          up_rdata <= 32'd0;
  ...
  assign fps = up_fps;
  assign convert_uyvy_to_yuyv = up_convert_uyvy_to_yuyv;
