...
module rtp_engine #(
  ...
  reg                        m_axis_tlast_cam_last = 1'b0;
  ...
  always @(posedge aclk) begin
    if (!aresetn) begin
      ...
      if (rtp_transm_state == ETH_HDR) begin
        ...
        if (counter_data_transm == 0 && m_axis_tlast_cam_last != 1 && m_axis_tready) begin
  ...
  always @(posedge aclk) begin
    if (!aresetn) begin
      m_axis_tlast_cam_r <= 1'b0;
      m_axis_tlast_cam_last <= 1'b0;
    end else begin
      m_axis_tlast_cam_r <= m_axis_tlast_cam;
      m_axis_tlast_cam_last <= m_axis_tlast_cam_r;
    end
  end
  ...
  assign m_axis_tvalid = m_axis_cam_tvalid_r | m_axis_tlast_cam_last;
  assign m_axis_tlast = m_axis_tlast_cam_last;
  ...
  // end of ip hdr in the next cycle
  ...
  assign end_of_payload_tr = m_axis_tlast_cam_last;
