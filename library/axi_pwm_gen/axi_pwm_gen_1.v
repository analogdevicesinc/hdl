...
module axi_pwm_gen_1 #(
  ...
  assign ext_sync_align = (EXT_SYNC_PHASE_ALIGN) ? ext_sync : 1'b0;
  ...
  always @(posedge clk) begin
    if (rstn == 1'b0) begin
      ...
      if (load_config) begin
        pulse_period_read <= pulse_period;
        pulse_width_read <= pulse_width;
        if (FORCE_ALIGN == 1 && START_AT_SYNC == 1) begin
          pulse_period_d <= pulse_period;
          pulse_width_d <= pulse_width;
        end
      end
  ...
  generate
    if (FORCE_ALIGN == 1) begin
      assign align_period = ext_sync_align == 1 || run_sm_next <= ARM || end_of_period == 1'b1;
      assign align_pulse =  ext_sync_align;
    end else begin
      assign align_period = run_sm_next <= ARM || end_of_period == 1'b1;
      assign align_pulse =  1'b0;
    end
  endgenerate
  ...
  assign start_at_sync = START_AT_SYNC == 1 ? ~run_sm & ~sync : 1'b0;
  ...
  always @ (posedge clk) begin
    if (rstn == 1'b0 || end_of_pulse == 1'b1 || align_pulse == 1) begin
      pulse_i <= 1'b0;
    end else if (pulse_start & pulse_enable) begin
      pulse_i <= 1'b1;
    end
  end
