...
set_property -dict [list \
  ...
  "tooltip" "If active the external sync will be driven by clk_ext_sync, a clock faster than the rest of logic. Also, the ext_sync should not be held high for more than a clk_ext_sync period" \
  "widget" "checkBox" \
] [ipgui::get_guiparamspec -name "EXT_SYNC_FASTER_CLK" -component $cc]
  set_property enablement_tcl_expr {expr $PWM_EXT_SYNC == 1 && $EXT_ASYNC_SYNC == 1} [ipx::get_user_parameters EXT_SYNC_FASTER_CLK -of_objects $cc]
...
set_property -dict [list \
  ...
  "tooltip" "If active the ext_sync will trigger a phase align at each neg-edge. Otherwise the phase align will be armed by a load config toggle" \
  "widget" "checkBox" \
...
set_property -dict [list \
  ...
  "tooltip" "If active the current active pulses are immediately stopped and realigned. Otherwise, the synchronized pulses will start only after all running pulse periods end." \
  "widget" "checkBox" \
...
set_property -dict [list \
  ...
  "tooltip" "If active, the pulses will start after the trigger event. Otherwise each pulse will start after a period equal to the one for which it is set ." \
  "widget" "checkBox" \
