...
localparam SRC_ADDRESS_WIDTH = LENGTH_WIDTH - $clog2(SRC_DATA_WIDTH/8);
localparam DST_ADDRESS_WIDTH = LENGTH_WIDTH - $clog2(DST_DATA_WIDTH/8);
...
always @(posedge s_axis_aclk) begin
  if (wr_request_valid & wr_request_ready)
    wr_length <= wr_request_length >> $clog2(SRC_DATA_WIDTH/8);
end
