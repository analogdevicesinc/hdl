...
proc p_alt_serdes {} {
  ...
  ## cyclone V, clock only - others are megafunctions
  ...
  if {$m_device_family == "Arria 10"} {
    ...
    if {($m_mode == "IN") && ($m_serdes_factor == 2)} {
      add_instance alt_serdes_in altera_gpio
      set_instance_parameter_value alt_serdes_in {PIN_TYPE_GUI} {Input}
      set_instance_parameter_value alt_serdes_in {SIZE} {1}
      set_instance_parameter_value alt_serdes_in {gui_diff_buff} {0}
      set_instance_parameter_value alt_serdes_in {gui_io_reg_mode} {DDIO}
      add_interface clk conduit end
      set_interface_property clk EXPORT_OF alt_serdes_in.ck
      add_interface din conduit end
      set_interface_property din EXPORT_OF alt_serdes_in.din
      add_interface pad_out conduit end
      set_interface_property pad_out EXPORT_OF alt_serdes_in.pad_out
      return
    }
    if {($m_mode == "OUT") && ($m_serdes_factor == 2)} {
