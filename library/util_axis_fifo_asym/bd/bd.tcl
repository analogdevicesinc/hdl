...
proc init {cellpath otherInfo} {
	...
	bd::mark_propagate_overrideable $ip \
		"ASYNC_CLK S_DATA_WIDTH M_DATA_WIDTH TLAST_EN TKEEP_EN TSTRB_EN TUSER_EN TID_EN TDEST_EN TUSER_WIDTH TID_WIDTH TDEST_WIDTH"
...
proc parameter_set {cellpath} {
	...
	if {$interface_pin_source != {}} {
		...
		set_property_propagated TUSER_EN [expr $tuser_width_slave > 1] $ip
	...
	# secondary signal checkings
	set pin_source [find_bd_objs -relation connected_to [get_bd_pins -of_objects $ip -filter {NAME =~ s_axis_data}]]
	if {$pin_source != {}} {
		set data_width [get_property "LEFT" $pin_source]
		set_property "CONFIG.S_DATA_WIDTH" [expr $data_width + 1] $ip
	}

	set pin_source [find_bd_objs -relation connected_to [get_bd_pins -of_objects $ip -filter {NAME =~ m_axis_data}]]
	if {$pin_source != {}} {
		set data_width [get_property "LEFT" $pin_source]
		set_property "CONFIG.M_DATA_WIDTH" [expr $data_width + 1] $ip
	}
