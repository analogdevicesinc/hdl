...
module frame_combiner #(
  ...
  // During first frame don't wait for data from the reader DMAC
...
//  reg [8:0] dly_cnt = 'h0;
//  reg s_in_axis_user_d = 1'b0;
//  always @(posedge s_fb_in_axis_aclk) begin
//    s_in_axis_user_d <= s_in_axis_user;
//    if (s_in_axis_user & ~s_in_axis_user_d) begin
//      dly_cnt <= {9{1'b1}};
//    end else if (|dly_cnt) begin
//      dly_cnt <= dly_cnt - 1;
//    end
//  end
//  assign fsync_fb = dly_cnt == 9'h1;
  ...
  generate
    for (i=0; i< PIXELS_PER_CLOCK; i = i+1) begin : genloop
      for (j=0; j< 4; j = j+1) begin : genloop
        ...
        wire [8:0] data;
        //assign data = {1'b0,s_in_axis_data[LSB +: 8]} + {1'b0,s_fb_in_axis_data[LSB +: 8]};   1/2 + 1/2
        //assign combined_data[LSB +: 8] = data[8:1];
  ...
  always @(posedge s_axi_aclk) begin
    if (s_axi_aresetn == 1'b0) begin
      ...
      if (up_wreq == 1'b1) begin
        case (up_waddr)
        9'h100: {ctrl_select, ctrl_enable} <= up_wdata[1:0];
        endcase
      end
    end
  end
  ...
  always @(posedge s_axi_aclk) begin
    if (up_rreq == 1'b1) begin
      case (up_raddr)
        9'h100: up_rdata <= {ctrl_select, ctrl_enable};
        default: up_rdata <= 'h0;
      endcase
    end
  end
