...
proc data_offload_compose {} {
  ...
  add_instance sys_clock altera_clock_bridge
  add_interface sys_clk clock sink
  set_interface_property sys_clk EXPORT_OF sys_clock.in_clk
  add_instance sys_resetn altera_reset_bridge
  set_instance_parameter_value sys_resetn {ACTIVE_LOW_RESET} {1}
  add_interface sys_resetn reset sink
  set_interface_property sys_resetn EXPORT_OF sys_resetn.in_reset
  add_connection sys_clock.out_clk sys_resetn.clk
  ...
  if {$mem_type == 0} {
    ...
    ###########################################################################
    # Bridge instance for the external DDR memory controller
    # NOTE: The DDR/HBM instantiation should be in project's system_qsys.tcl file
    ###########################################################################

    add_instance storage_unit util_hbm
    set_instance_parameter_value storage_unit {LENGTH_WIDTH} [log2 $mem_size]
    set_instance_parameter_value storage_unit {SRC_DATA_WIDTH} $source_dwidth
    set_instance_parameter_value storage_unit {DST_DATA_WIDTH} $destination_dwidth
    set_instance_parameter_value storage_unit {AXI_DATA_WIDTH} $axi_data_width
    set_instance_parameter_value storage_unit {TX_RX_N} $datapath_type
    set_instance_parameter_value storage_unit {MEM_TYPE} $mem_type
    set_instance_parameter_value storage_unit {AXI_PROTOCOL} 0
    set_instance_parameter_value storage_unit {NUM_M} 1
    ...
    add_interface ddr_calib_done conduit end
    set_interface_property ddr_calib_done EXPORT_OF i_data_offload.if_ddr_calib_done

    add_connection storage_unit.if_wr_overflow i_data_offload.if_wr_overflow
    add_connection storage_unit.if_rd_underflow i_data_offload.if_rd_underflow
  }
  ...
  add_connection sys_clock.out_clk i_data_offload.s_axi_clock
  add_connection sys_resetn.out_reset i_data_offload.s_axi_reset
