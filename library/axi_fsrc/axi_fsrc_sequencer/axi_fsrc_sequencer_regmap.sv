...
//   0x2  RW  scratch
//   0x3  RO  core magic
//   0x4  RW  gpio change count
//   0x5  RW  first trigger count, four bits per trigger
//   0x6  RW  [0] start, [1] enable, [4] non fsrc delay enable,
//            [16+] tx accumulator reset count
//   0x7  RW  [0] external trigger enable, [16+] rx delay count
//   0x8  RW  Apollo control word, low half
//   0x9  RW  Apollo control word, high half
//   0x10 RW  manual trigger out, one bit per trigger
//   0x11 RW  second trigger count, four bits per trigger
//
// 0x11 and the upper half of 0x7 are additions; every address the driver already
...
module axi_fsrc_sequencer_regmap #(
  ...
  output [COUNTER_WIDTH-1:0] reg_o_seq_tx_accum_reset_cnt,
  ...
  wire [31:0] trig_ctrl;
  wire [31:0] seq_ctrl_5;
  wire [63:0] seq_gpio_w;
    ...
    if (up_rstn == 0) begin
      ...
      if (up_rreq == 1'b1) begin
        case (up_raddr)
          ...
          14'h11:  up_rdata <= up_seq_ctrl_5_s;
          default: up_rdata <= 0;
        endcase
      end else begin
        up_rdata <= 32'd0;
  ...
  generate
    for (i = 0; i < NUM_TRIG; i++) begin
      ...
      assign reg_o_second_trig_cnt[i] = seq_ctrl_5[(i*4) +: COUNTER_WIDTH];
    end
  endgenerate
  ...
  assign seq_gpio_w = {seq_gpio_w_2, seq_gpio_w_1};
  assign reg_o_dut_seq_gpio_w = seq_gpio_w[CTRL_WIDTH-1:0];
