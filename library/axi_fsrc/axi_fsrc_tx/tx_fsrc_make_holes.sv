...
module tx_fsrc_make_holes #(
  ...
  for(jj=0;jj<NUM_WORDS;jj=jj+1) begin : move_holes_gen
    // Continuous assigns rather than an always_comb writing one element of a
    // packed array, which not every simulator accepts.
    if(jj==0) begin : first
      assign non_holes_cnt_total_per_word[jj] = !holes_data_d[1][0];
    end else begin : rest
      assign non_holes_cnt_total_per_word[jj] =
        non_holes_cnt_total_per_word[jj-1] + !holes_data_d[1][jj];
    end
