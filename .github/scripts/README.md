...
* runs the full guideline check on Verilog (.v) and SystemVerilog (.sv) files,
except those containing "tb" in their path/name (testbenches)
* testbench files (`.v`/`.sv` containing "tb") are excluded from the full
structural check, but still receive the lightweight license-only check
* also runs on non-HDL files (`.tcl`, `.ttcl`, `.xdc`, `.sdc`, `.pdc`, `.py`,
`.sh`, `.pl`, `.vh`), for which only a lightweight license-only check is
performed — see [Non-HDL file license check](#11-non-hdl-and-testbench-file-license-check)
* uses Python 3.x
...
Besides the full guideline check on regular HDL sources, the script also runs a
**lightweight license-only check** on two other groups of files:

* **non-HDL files** with the following extensions:
  ```
  .tcl, .ttcl, .xdc, .sdc, .pdc, .py, .sh, .pl, .vh
  ```
* **testbench HDL files** — `.v`/`.sv` files containing `tb` in their name.
  These are excluded from the full structural check, but their license header
  is still verified.

For all of these, only the copyright-year check ([1.1](#11-copyright-year-check))
and license-body validation ([1.2](#12-license-body-validation)) are run. None of
the RTL-specific structural checks (empty lines, indentation, module/instance
formatting, typedefs, packages, etc.) apply to them.

Files in the `copyright_skip_list` are skipped here as well.

_When each group is checked_:
* **testbench files** are checked both in the argument-less/`-e` full-repo scan
  (all `tb` `.v`/`.sv` files under `library/` and `projects/`) and when passed
  explicitly via `-m`/`-me`/`-p`/`-pe`.
* **non-HDL files** are checked only when passed explicitly through
  `-m`/`-me`/`-p`/`-pe` (the mode the CI uses to check the files changed in a
  pull request); the full-repo scan does not walk them.

If `-e`/`-me`/`-pe` is added, the copyright years of these files can be updated,
just like for HDL files.
